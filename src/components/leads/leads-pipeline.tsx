...
import { deleteAllLeads } from "@/lib/leads/actions";
import {
  LEAD_CATEGORIES,
  LEAD_STATUSES,
  leadProduct,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <a
            href={`${basePath}/export?${buildParams({})}`}
      ...
      {searchParams.imported && (
        <div className="text-sm text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-lg px-3.5 py-2.5">
          ...
          {searchParams.imported === "1" ? "lead" : "leads"} into {label}.
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        >
          ...
          <div className="relative w-full sm:w-56">
            ...
            <input
              ...
              placeholder={`Search ${label} leads...`}
        ...
        <div className="flex items-center gap-1 text-sm bg-white border border-slate-200 rounded-full p-1">
          <Link
            href={`${basePath}?${buildParams({ view: "board" })}`}
          ...
          <Link
            href={`${basePath}?${buildParams({ view: "list" })}`}
            className={`px-3 py-1 rounded-full transition ${
              view === "list"
                ? "bg-gradient-to-r from-brand to-brand-indigo text-white"
                : "text-slate-600 hover:text-brand"
